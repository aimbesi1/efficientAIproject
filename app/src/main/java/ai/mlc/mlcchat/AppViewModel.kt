...
import ai.mlc.mlcllm.appendPromptResultToCsv
import ai.mlc.mlcllm.computePerplexityFromLogProbs
import ai.mlc.mlcllm.readEnergyCounterMicroWh
import ai.mlc.mlcllm.runtimeUsedBytes
    ...
    inner class ChatState {
        ...
        fun requestGenerate(prompt: String, activity: Activity) {
            ...
            executorService.submit {
                ...
                viewModelScope.launch {
                    // --- METRIC: start measurements ---
                    val startNs = System.nanoTime()
                    val cpuStartNs = Debug.threadCpuTimeNanos()
                    val memStartBytes = runtimeUsedBytes()
                    val energyStartMicroWh = readEnergyCounterMicroWh(activity) // nullable; device dependent

                    // used to capture usage from the stream (last non-null usage)
                    var finalUsage: OpenAIProtocol.CompletionUsage? = null

                    var finishReasonLength = false
                    var streamingText = ""
                    var firstTokenNs: Long? = null
                    var energyEndMicroWh: Long? = null
                    val tokenLogProbs = mutableListOf<OpenAIProtocol.LogProbsContent>()

                    val runId = UUID.randomUUID().toString()
                    val startEpoch = System.currentTimeMillis()

                    var logProbsFound = false

                    for (res in engine.chat.completions.create(
                        messages = historyMessages,
//                        stream = false, // crashes
...
//                        top_p = 0.4F
                        ...
                        Log.v("requestGenerate","raw response JSON: ${res.toString()}")
                        // record usage if present in this chunk
                        res.usage?.let { finalUsage = it }

                        for (choice in res.choices) {
                            // delta content - streaming token arrived
                            choice.delta.content?.let { contentDelta ->
                                val text = contentDelta.asText()
                                // mark first-token time when streamingText was previously empty and a delta arrives
                                if (streamingText.isEmpty() && text.isNotEmpty() && firstTokenNs == null) {
                                    // set the var directly (no smart-cast here)
                                    firstTokenNs = System.nanoTime()
                                }
                                streamingText += text
                            }

                            choice.logprobs?.content?.let { logprobContentList ->
                                logProbsFound = true
                                for (lp in logprobContentList) {
                                    tokenLogProbs.add(
                                        OpenAIProtocol.LogProbsContent(
                                            token = lp.token,
                                            logprob = lp.logprob,
                                            bytes = lp.bytes,
                        ...
                        // update small runtime report if usage present (you had this)
                        res.usage?.let { usage ->
                            finalUsage = usage
                    ...
                    } // end streaming for-loop
                    if (logProbsFound) {
                        Log.v("requestGenerate", "Found logprobs")
                    } else {
                        Log.e("requestGenerate", "No logprobs")
                    }
                    val endEpoch = System.currentTimeMillis()
                    // --- METRIC: end measurements ---
                    val endNs = System.nanoTime()
                    val cpuEndNs = Debug.threadCpuTimeNanos()
                    val memEndBytes = runtimeUsedBytes()
                    // read energy after streaming finished (take a snapshot)
                    energyEndMicroWh = readEnergyCounterMicroWh(activity) // may be null

                    // --- SNAPSHOT captured mutable vars into stable vals ---
                    val finalUsageSnapshot = finalUsage // stable val for later use
                    val firstTokenNsSnapshot = firstTokenNs // stable val for later use
                    val energyStartSnapshot = energyStartMicroWh
                    val energyEndSnapshot = energyEndMicroWh

                    // CPU and mem deltas
                    val totalCpuMs = (cpuEndNs - cpuStartNs) / 1_000_000.0f
                    val memDeltaMb = (memEndBytes - memStartBytes).toFloat() / (1024f * 1024f)
                    val totalLatencySec = (endNs - startNs) / 1_000_000_000.0f

                    // use snapshot safely — avoids smart-cast problems
                    val timeToFirstTokenSec = firstTokenNsSnapshot?.let { (it - startNs) / 1_000_000_000.0f } ?: totalLatencySec

                    // usage-based token counts (if stream included usage at least once)
                    val promptTokens = finalUsageSnapshot?.prompt_tokens ?: 0
                    val completionTokens = finalUsageSnapshot?.completion_tokens ?: 0
                    val totalTokens = finalUsageSnapshot?.total_tokens ?: (promptTokens + completionTokens)

                    val prefillTokPerS = finalUsageSnapshot?.extra?.prefill_tokens_per_s ?: 0f
                    val decodeTokPerS = finalUsageSnapshot?.extra?.decode_tokens_per_s ?: 0f
                    val numPrefillTokens = finalUsageSnapshot?.extra?.num_prefill_tokens ?: 0

                    // throughput: completion tokens per second (guard against zero)
                    val throughputTps = if (totalLatencySec > 0f) (completionTokens.toFloat() / totalLatencySec) else 0f

                    // This energy measurement method is never going to work. Use Power Profiler instead.
                    val energyConsumedMicroWh: Float? = if (energyStartSnapshot != null && energyEndSnapshot != null) {
                        (energyStartSnapshot - energyEndSnapshot).toFloat()
                    } else null

                    val avgPowerW: Float? = energyConsumedMicroWh?.let { e ->
                        if (totalLatencySec > 0.0f) {
                            (e * 3600.0f) / (1_000_000.0f * totalLatencySec) // W
                        } else null
                    }

                    // build final assistant response text (you already add it to history below - reuse streamingText)
                    ...
                    // Perplexity left as before — adapt if you have logprobs in responses
                    var perplexityScore: Float? = computePerplexityFromLogProbs(tokenLogProbs)

                    // Build PromptResultData
                    val result = OpenAIProtocol.PromptResultData(
                        run_id = runId,
                        prompt = prompt,
                        ...
                        prefill_tokens_per_s = prefillTokPerS,
                        decode_tokens_per_s = decodeTokPerS,
                        num_prefill_tokens = numPrefillTokens,
                        total_latency = totalLatencySec,
                        time_to_first_token = timeToFirstTokenSec,
                        throughput_tps = throughputTps,
                        energy_consumed_uWh = energyConsumedMicroWh,
                        avg_power_w = avgPowerW,
                        cpu_time_ms = totalCpuMs,
                    ...
                    // Save to CSV (app-private storage)
                    try {
                        appendPromptResultToCsv(activity, result)
                    } catch (e: Exception) {
                        Log.e("requestGenerate", "Failed to write prompt result CSV: ${e.message}", e)
                    }

                    // restore UI state as before
                    if (modelChatState.value == ModelChatState.Generating) switchToReady()
                }
